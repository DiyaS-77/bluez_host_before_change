...
class TestControllerUI(QWidget):
    ...
    def __init__(self,bluetooth_device_manager,back_callback=None):
        """
        ...
        Args:
            ...
            bluetooth_device_manager: An instance of BluetoothDeviceManager class used for handling HCI dump logs.
    ...
    def controller_ui(self):
        ...
        self.bluetooth_device_manager.start_dump_logs(interface=self.bluetooth_device_manager.interface)
    ...
    def run_hci_cmd(self, text_selected):
        ...
        for parameter in parameters:
            ...
            if 'Connection_Handle' in key:
                ...
                combo_box.addItems(list(self.bluetooth_device_manager.get_connection_handles().keys()))
    ...
    def execute_hci_cmd(self):
        ...
        """
        parameters = []
        self.bluetooth_device_manager.get_connection_handles()

        for parameter in getattr(hci, self.ocf.lower().replace(' ', '_'))[self.ogf][1]:
            key = list(parameter.keys())[0]

            if isinstance(getattr(self, key), QComboBox):
                parameters.append(self.bluetooth_device_manager.handles[self.handle])
                self.handle = None
                continue
        ...
        self.bluetooth_device_manager.run_hci_cmd(self.ocf, self.ogf, parameters)
